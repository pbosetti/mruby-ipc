...
class String
  ...
  def matches
    matches = []
    re_match = self.match(REGEXP_PATTERN)
    1.upto(5) do |i|
      matches << (re_match[i] == "" ? nil : re_match[i])
    end
    [matches]
  end
  ...
  #
  # @example Set various colors:
  #   puts "This is blue".colorize(:blue)
  #   puts "This is light blue".colorize(:light_blue)
  #   puts "This is also blue".colorize(:color => :blue)
  #   puts "This is light blue with red background".colorize(:color => :light_blue, :background => :red)
  #   puts "This is light blue with red background".colorize(:light_blue ).colorize( :background => :red)
  # @example Commodity methods are automatically made available corresponding to {String::MODES} and {String::COLORS}:
  ...
  # @return [String] copy of self with color codes
  def colorize(params)
    matches.inject("") do |str, match|
  ...
  # @return [String] copy of self with color codes removed
  def uncolorize
    matches.inject("") do |str, match|
      str << (match[3] || match[4])
    end
  ...
  # @return [Bool] 'true' only if self contains color codes
  def colorized?
  ...
  class << self
    ...
    def color_matrix(txt = '[X]')
      ...
      String.colors.reverse.each_with_index do |back, index|
        puts "#{"|".ljust(txt.length)*(size-index)} < #{back}"
      end
      ''
    end
